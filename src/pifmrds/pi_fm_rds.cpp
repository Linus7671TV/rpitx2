...
int tx(tx_data *data) {
    ...
    if(data->log) {
        if(drds == 1) {
            ...
            int active_di = (data->di != -1 ? data->di : (dstereo == 1 ? 0 : 1));
            printf("PI: %04X, ECC: %02X, LIC: %02X, PS: \"%s\", DI: %d.\n", data->pi, data->ecc, data->lic, data->ps, active_di);
...
int main(int argc, char **argv) {
    ...
    for(int i=1; i<argc; i++) {
        ...
        if((strcmp("-audio", arg)==0) && param != NULL) {
            ...
            i++;
            data.di = atoi(param);
            if(data.di < 0 || data.di > 15) {
                fatal("Wrong DI range, range is 0-15\n");
            }
            ...
            "                  [-ps ps_text] [-lps long_ps_text] [-rt rt_text] [-ptyn ptyn_text] [-ctl control_pipe] [-pty program_type] [-di di_code]\n", arg);
        }
    }
