...
ResultAndArg poll_control_pipe(int log) {
	...
	if(strlen(fifo) > 3 && fifo[2] == ' ') {
		...
		if(fifo[0] == 'L' && fifo[1] == 'P' && fifo[2] == 'S') {
		...
		else if(fifo[0] == 'P' && fifo[1] == 'T' && fifo[2] == 'Y') {
			int pty = atoi(arg);
			if (pty >= 0 && pty <= 31) {
				set_rds_pty(pty);
				if(log==1) {
		...
		} else if(fifo[0] == 'P' && fifo[1] == 'T' && fifo[2] == 'N') {
			if (strlen(arg) > 8) arg[8] = 0;
			set_rds_ptyn(arg);
			if(log==1) printf("PTYN set to: \"%s\"\n", arg);
			resarg.res = CONTROL_PIPE_PTYN_SET;
			...
			resarg.res = CONTROL_PIPE_GENERIC_SET;
		} else if(fifo[0] == 'L' && fifo[1] == 'I' && fifo[2] == 'C') {
			if (strlen(arg) > 2) arg[2] = 0;
			set_rds_lic((uint16_t) strtol(arg, NULL, 16));
			if(log==1) printf("LIC set to: \"%s\"\n", arg);
			resarg.res = CONTROL_PIPE_LIC_SET;
